...
<form id="form" action="<?php echo url_for('lote/'.($form->getObject()->isNew() ? 'create' : 'pack').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
<?php include_partial('form', array('form' => $form)) ?>
  ...
  &nbsp;<span class="volver"><a href="<?php echo url_for('lote/index') ?>">Volver</a></span>
          ...
          <?php endif; ?>
          &nbsp;-&nbsp;<input type="submit" value="Empacar" />
  </div>
</form>